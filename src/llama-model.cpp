...
void llama_model::load_stats(llama_model_loader & ml) {
    ...
    // Cost to store: 112 bytes of KV metadata — zero impact on bpw.
    ...
    int64_t key_centroids = gguf_find_key(meta, "turbo_quant.tq3_rvq.centroids");
    if (key_cb2 >= 0 && key_cb3 >= 0 &&
        ...
        if (key_centroids >= 0 && gguf_get_arr_n(meta, key_centroids) == 8) {
            const float * centroids = (const float *) gguf_get_arr_data(meta, key_centroids);
            tq3_rvq_set_centroids(centroids);
        }
